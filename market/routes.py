    ...
    if request.method == "POST":
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object):
                ...
                flash("Congrats for the purchased!", category='success')
            else:
                flash("No budget enouth to purchase", category='danger')
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash("Congrats for the sell!", category='success')
            else:
                flash("No item to sell", category='danger')
...
def login_page():
    ...
    if form.validate_on_submit():
        ...
        if attempted_user and attempted_user.check_password_correction(attempted_password=form.password.data):
            ...
            flash(f'Success Login {attempted_user.username}', category='success')
            return redirect(url_for('market_page'))
        else:
            flash('erro no login', category='danger')
    return render_template('login.html', form=form)

@app.route('/logout')
def logout_page():
    logout_user()
    flash('You habe been logged out', category='info')
